...
export const PWAInstallButton: React.FC<PWAInstallButtonProps> = ({ variant = 'nav' }) => {
  ...
  return (
    <>
      <div className="rounded-xl border border-[#d4af37]/30 bg-[#16181f]/95 p-4 shadow-xl backdrop-blur-sm flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3 text-left w-full sm:w-auto">
          ...
          <div>
            <h4 className="text-sm font-semibold text-white">Instale o App Barbearia Liberdade</h4>
            <p className="text-xs text-neutral-400">Agende cortes mais rápido direto da tela do seu celular</p>
          </div>
        </div>
        <button
          onClick={handleInstallClick}
          className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[#d4af37] to-[#aa8222] px-4 py-2 text-xs font-bold text-[#0d0e11] hover:brightness-110 transition shadow-md whitespace-nowrap"
        >
